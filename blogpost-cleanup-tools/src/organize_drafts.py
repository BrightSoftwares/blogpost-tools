...
# An empty dataframe
df = pd.DataFrame(columns=['image', 'author', 'content', 'category', 'date', 'description', 'layout', 'pretified', 'ref', 'tags', 'title', 'silot_terms', 'keyword_suggestion', 'post_inspiration', 'transcribed', 'youtube_video', 'youtube_video_id'])
...
for entry in entries:
    try:
        ...
        #print("Adding")
        df.loc[len(df)] = [image, author, content, category, date, description, layout, pretified, ref, tags_str, title, silot_terms, keyword_suggestion, post_inspiration, transcribed, youtube_video, youtube_video_id]
