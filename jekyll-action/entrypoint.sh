...
# It can be provided, or detected through API or inferred from the repo name, which is a bit of a legacy behavior.
...
echo "::debug::Bundle config set successfully"
bundle install
...
if [ -n "$INPUT_ALGOLIA_API_KEY" ]; then
  echo "Uploading search data to Algolia."
  ALGOLIA_API_KEY=$INPUT_ALGOLIA_API_KEY bundle exec jekyll algolia
fi
