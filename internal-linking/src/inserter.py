...
body, inserting ``[[target-slug]]`` (when the anchor text already equals
the target's slug) or ``[[target-slug|anchor text]]`` (otherwise).
...
def insert_wikilinks(
    ...
    for opp in sorted_opps:
        ...
        if normalize(original_text) == normalize(opp.target_slug.replace("-", " ")):
            wikilink = f"[[{opp.target_slug}]]"
        else:
            wikilink = f"[[{opp.target_slug}|{original_text}]]"
