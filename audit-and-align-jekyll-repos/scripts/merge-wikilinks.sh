#!/bin/bash
#
# Merge WikiLinks plugin branches to main/master
#
...
echo "=========================================="
echo "Merge WikiLinks Plugin Branches"
echo "=========================================="
echo ""
...
for repo in "${ALL_REPOS[@]}"; do
    ...
    merge_result=$(curl -s -X POST \
        ...
        -d "{\"base\":\"$default_branch\",\"head\":\"$BRANCH_NAME\",\"commit_message\":\"Add jekyll-wikilinks-v2 plugin\\n\\n- Converts WikiLinks syntax [[Page Name]] to proper links\\n- Supports slug matching (post filenames)\\n- Supports anchor links [[slug#anchor|text]]\\n- Supports alias syntax [[slug|display text]]\\n- Compatible with Jekyll 4.3.4+\\n- All workflows tested and passing\\n\\nBranch: $BRANCH_NAME\"}" \
...
if [[ $success -gt 0 ]] || [[ $skipped -gt 0 ]]; then
    echo "🎉 WikiLinks plugin deployed to production!"
    echo ""
    echo "All repositories now have:"
    echo "  - WikiLinks plugin with slug/anchor support"
