#!/bin/bash
#
# Switch all Jekyll repo workflows from ubuntu-latest to self-hosted
#
...
echo "=========================================="
echo "Switching to self-hosted runners"
echo "=========================================="
echo ""
...
for repo in "${ALL_REPOS[@]}"; do
    ...
    if echo "$content" | grep -q "runs-on: self-hosted"; then
        log_info "Already using self-hosted"
        echo ""
        continue
    fi

    # Update: change ubuntu-latest to self-hosted with comment
    # Handle different formats:
    # 1. runs-on: ubuntu-latest
    # 2. # runs-on: ubuntu-latest\n    runs-on: self-hosted (already correct format)

    new_content=$(echo "$content" | sed 's/runs-on: ubuntu-latest/# runs-on: ubuntu-latest\n    runs-on: self-hosted/')
    ...
    result=$(curl -s -X PUT \
        ...
        -d "{\"message\":\"Switch to self-hosted runner (ubuntu-latest commented)\",\"content\":\"$encoded\",\"sha\":\"$sha\",\"branch\":\"$default_branch\"}" \
